...
## People who watched Star Wars IV also watched ...

Calculate movies that most often occur with other movie. For each movie, calculate the percentage of the other movie raters who also rated that movie. Order with the highest percentage first, and submit the top 5. Select "Star.Wars.IV...A.New.Hope" movie by default.

```{r fig.width=7, fig.height=6}
StarWars_count <- sum((df[,1] >= 0), na.rm = T) 
# Filter first the rows with a rating for the default movie and delete the column of the default movie to generate the (intersection x + y )
df_StarWars_movie <- df[!is.na(df$`Star Wars IV - A New Hope`),][,-1]
top_occurences <- data.table()
for (i in 1:ncol(df_StarWars_movie)){
  movie <- df_StarWars_movie[,i]
  # ratings of the movie i 
  other_movie = sum(movie >= 0, na.rm = T) # automatically retrieves the intersection as the df_StarWars_movie only contains ratings user watched both movie i and Star Wars
  occ_percentage = other_movie / StarWars_count # devide intersection x + y by y 
  top_occurences <- rbind(top_occurences, data.table(
                               movie_name = colnames(df_StarWars_movie)[i],
                               occ_percentage = occ_percentage));
}
top_occurences[order(-occ_percentage)][1:5]
```
