...
```{r}
edata <- read.csv("data/transactions_detail.csv",stringsAsFactors=FALSE, sep=',')
```
...
```{r inspect rule}
inspect(basket_rules, by = "lift", decreasing=TRUE)
```
...
```{r}
inspect(subset(basket_rules.refined, subset = items %in% "pastry"))
```
